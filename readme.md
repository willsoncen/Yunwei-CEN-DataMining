...
3. **命名仓库**: 输入项目名称 `Yunwei-CEN-DataMining`。
4. **创建**: 完成仓库信息填写后，点击 `Create repository`。

## 三、上传PPT至项目

1. **返回主页**: 登录GitHub后，回到GitHub首页。
2. **访问个人仓库**: 点击右上角的个人头像，选择 `Your repositories`。
3. **选择项目**: 点击进入 `Yunwei-CEN-DataMining` 项目页面。
4. **添加文件**: 在页面顶部导航栏，点击 `Add file`，然后选择 `Upload files`，浏览并上传你的PPT文件。
